...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .header { margin-bottom: 10px; border-bottom: 2px solid #3b82f6; padding-bottom: 8px; }
        .header h1 { font-size: 15px; font-weight: 700; color: #1e3a5f; margin-bottom: 1px; }
        ...
        .grupo-header { background: #1e3a5f; color: #fff; padding: 4px 6px; font-size: 8.5px; font-weight: 700; border-radius: 2px 2px 0 0; }
        ...
        .subtotal-row td { background: #eef2ff !important; font-weight: 700; font-size: 8px; border-top: 2px solid #c7d2fe; }
        .total-row td { background: #1e3a5f !important; color: #fff !important; font-weight: 700; font-size: 8.5px; border: none; padding: 4px; }
...
<body>
            ...
            <tr>
                ...
                <td width="20%">
                    <div class="box box-success">
                        ...
                        <span class="value value-success">R$ {{ number_format($totais['recebido'], 2, ',', '.') }}</span>
                    </div>
                </td>
                <td width="20%">
        ...
        <table class="dados">
            <thead>
                <tr>
                    ...
                    <th style="width: 10%;" class="text-right">Recebido</th>
            ...
            <tbody>
                ...
                @endphp
                <tr>
                    <td>{{ Str::limit($c->cliente->nome ?? $c->cliente->razao_social ?? '—', 28) }}</td>
                    <td>
                        {{ Str::limit($c->descricao, 32) }}
                        @if($c->total_parcelas > 1)
                            <span class="text-muted">({{ $c->numero_parcela }}/{{ $c->total_parcelas }})</span>
                        @endif
                    </td>
                    ...
                    <td class="text-right text-green">R$ {{ number_format($c->valor_recebido, 2, ',', '.') }}</td>
                ...
                <tr class="subtotal-row">
                    <td colspan="4" class="text-right">Subtotal {{ $nomeGrupo }}:</td>
                    ...
                    <td class="text-right">R$ {{ number_format($grupo['subtotais']['recebido'], 2, ',', '.') }}</td>
    ...
    <table class="dados">
        <tr class="total-row">
            <td style="width: 54%;" class="text-right">TOTAL GERAL ({{ $totalRegistros }} títulos):</td>
            ...
            <td style="width: 10%;" class="text-right">R$ {{ number_format($totais['recebido'], 2, ',', '.') }}</td>
