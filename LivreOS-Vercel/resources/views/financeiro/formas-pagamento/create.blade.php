...
@section('content')
<div class="mb-6">
    <h1 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Nova Forma de Pagamento</h1>
    <p class="text-gray-600 dark:text-gray-400">Cadastre uma nova forma de pagamento</p>
</div>

<div class="rounded-lg border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-800">
    <form action="{{ route('financeiro.formas-pagamento.store') }}" method="POST" class="p-6">
        @csrf
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select name="tipo" id="forma_tipo" required onchange="toggleAdquirentes()" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    <option value="pix" {{ old('tipo') === 'pix' ? 'selected' : '' }}>Pix</option>
                    ...
                    <option value="cartao_debito" {{ old('tipo') === 'cartao_debito' ? 'selected' : '' }}>Cartão de Débito</option>
                    ...
                    <option value="cheque" {{ old('tipo') === 'cheque' ? 'selected' : '' }}>Cheque</option>
                </select>
            </div>

            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Taxa Percentual (%)</label>
                <input type="number" step="0.01" name="taxa_percentual" value="{{ old('taxa_percentual', 0) }}" min="0" max="100" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>
            ...
            <div>
                ...
                <input type="number" name="dias_recebimento" value="{{ old('dias_recebimento', 0) }}" min="0" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>

            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Máximo de Parcelas</label>
                <input type="number" name="max_parcelas" value="{{ old('max_parcelas') }}" min="1" max="120" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>
            ...
            <div>
                ...
                <select name="conta_bancaria_id" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    ...
                    @foreach($contasBancarias ?? [] as $contaBancaria)
                        <option value="{{ $contaBancaria->id }}" {{ old('conta_bancaria_id') == $contaBancaria->id ? 'selected' : '' }}>{{ $contaBancaria->nome }}</option>
                    @endforeach
                </select>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Conta onde os pagamentos desta forma serão creditados por padrão</p>
            </div>

            <div id="pix_chaves_wrap" class="hidden md:col-span-2">
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/50">
                    <div class="mb-3 flex items-center justify-between gap-2">
                        <div>
                            ...
                            <p class="text-xs text-gray-600 dark:text-gray-400">Cadastre as chaves com conta vinculada e taxa por transação (opcional).</p>
                        </div>
            ...
            <div class="md:col-span-2 space-y-2">
                ...
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="permite_parcela" value="1" {{ old('permite_parcela') ? 'checked' : '' }} class="rounded border-gray-300 text-brand-500 focus:ring-brand-500">
        ...
        <?php do_action('financeiro.formas-pagamento.form.extra', null); ?>
        @endif
    ...
    const html = `
        <div class="grid grid-cols-1 gap-2 rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900 md:grid-cols-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="pix_chaves[${idx}][chave]" value="${data.chave || ''}" placeholder="CPF/CNPJ, e-mail, telefone ou aleatória" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>
            <div>
