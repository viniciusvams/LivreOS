...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h1 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Pagamentos Recorrentes</h1>
        <p class="text-gray-600 dark:text-gray-400">Recebimentos que se repetem (aluguel, assinatura, contrato). As contas a receber são geradas automaticamente.</p>
    </div>
    <a href="{{ route('financeiro.pagamentos-recorrentes.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">
...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-theme-sm dark:border-gray-800 dark:bg-gray-800">
    <form method="GET" action="{{ route('financeiro.pagamentos-recorrentes.index') }}" class="flex flex-wrap items-end gap-4">
        <div>
            <label class="mb-1.5 block text-xs font-medium text-gray-700 dark:text-gray-300">Cliente</label>
            <select name="cliente_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
        ...
        <div>
            ...
            <select name="tipo" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                ...
                <option value="assinatura" {{ request('tipo') === 'assinatura' ? 'selected' : '' }}>Assinatura</option>
        ...
        <a href="{{ route('financeiro.pagamentos-recorrentes.index') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">Limpar</a>
    </form>
</div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Vínculo</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse($recorrentes as $r)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td class="px-4 py-3 text-xs text-gray-600 dark:text-gray-400">
                            @if($r->controle_entidade_tipo || $r->controle_entidade_id || $r->controle_origem)
                                <div><strong>{{ $r->controle_entidade_tipo ?: '-' }}</strong></div>
                                <div>ID: {{ $r->controle_entidade_id ?: '-' }}</div>
                                @if($r->controle_origem)<div>Origem: {{ $r->controle_origem }}</div>@endif
                                @if($r->controle_referencia)<div>Ref: {{ $r->controle_referencia }}</div>@endif
                            @else
                        ...
                        <td class="px-4 py-3 text-center">
                            <a href="{{ route('financeiro.pagamentos-recorrentes.edit', $r) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400" title="Editar">
                            ...
                            <button type="button" onclick="abrirModalExcluirMotivo('{{ route('financeiro.pagamentos-recorrentes.destroy', $r) }}')" class="text-red-600 hover:text-red-800 dark:text-red-400" title="Excluir">
                ...
                @empty
                    <tr>
                        <td colspan="9" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">Nenhum pagamento recorrente cadastrado. Crie um para gerar contas a receber automaticamente (aluguel, assinatura, contrato).</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<p class="mt-4 text-xs text-gray-500 dark:text-gray-400">As contas a receber são criadas automaticamente pelo agendamento diário (comando <code>financeiro:gerar-recorrentes</code>). Em hospedagem compartilhada sem cron, habilite <strong>Configurações → Tarefas agendadas → Executar tarefas ao acessar o sistema</strong> para que a geração rode na primeira visita do dia.</p>

<x-modal-excluir-motivo action="" titulo="Excluir pagamento recorrente" descricao="Tem certeza que deseja excluir este pagamento recorrente? Informe o motivo." />
<script>
