...
<head>
    ...
    <title>Balancete - {{ $dataBalancete }}</title>
    <style>
...
<body>
    <h1>Balancete</h1>
    <p class="subtitle">Data: {{ \Carbon\Carbon::parse($dataBalancete)->format('d/m/Y') }}</p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $linha['nome'] }}</td>
                <td>{{ $linha['tipo'] }}</td>
                <td class="text-right">R$ {{ number_format($linha['saldo'] ?? 0, 2, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
