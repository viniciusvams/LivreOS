...
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Curva ABC de Clientes</h1>
        <p class="mt-1 text-gray-600 dark:text-gray-400">Classificação por receita: A (80% da receita), B (até 95%), C (restante). Período por competência de recebimento.</p>
    </div>
    <form method="get" action="{{ route('financeiro.relatorios.curva-abc-clientes') }}" class="flex flex-wrap items-end gap-3">
        <div>
...
<div class="mb-6 flex gap-4">
    <a href="{{ route('financeiro.relatorios.curva-abc-clientes.pdf', request()->query()) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
        ...
        <table class="w-full">
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Receita</th>
                    <th class="px-4 py-3 text-right text-xs font-medium uppercase text-gray-500 dark:text-gray-400">% Acum.</th>
                    <th class="px-4 py-3 text-center text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Classe</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td class="px-4 py-3 text-right text-sm font-medium text-gray-800 dark:text-white/90">R$ {{ number_format($item['total'], 2, ',', '.') }}</td>
                    <td class="px-4 py-3 text-right text-sm text-gray-600 dark:text-gray-400">{{ number_format($item['acumulado_pct'] ?? 0, 1, ',', '.') }}%</td>
