...
<body>
    <h1>{{ $empresaNome }}</h1>
    <p class="subtitulo">Cadastro de Produtos — Gerado em {{ now()->format('d/m/Y H:i') }}</p>
    ...
    @else
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $p->nome }}</td>
                    <td>{{ $p->codigo_sku ?? '—' }}</td>
                    <td>{{ ucfirst($p->formato ?? 'simples') }}</td>
                    <td>{{ $p->unidade ?? '—' }}</td>
                    <td class="text-right">{{ $p->preco_venda ? number_format($p->preco_venda, 2, ',', '.') : '—' }}</td>
                    <td class="text-right">{{ $p->estoque_quantidade !== null && $p->estoque_quantidade !== '' ? format_quantity($p->estoque_quantidade) : '—' }}</td>
        ...
        <p class="subtitulo" style="margin-top: 10px;">Total: {{ $produtos->count() }} produtos</p>
    @endif
