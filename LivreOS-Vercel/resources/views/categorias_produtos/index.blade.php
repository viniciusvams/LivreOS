...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Categorias de Produtos</h1>
        <p class="text-gray-600 dark:text-gray-400">Organize seus produtos por categoria</p>
    </div>
    <a href="{{ route('categorias-produtos.create') }}" class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">Nova Categoria</a>
</div>
...
<?php do_action('entity.index.before_table', 'categoria_produto'); ?>
@endif
        ...
        <table class="w-full border-collapse text-left text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700">
                    ...
                    <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $categoria->ativo ? 'Sim' : 'Não' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex gap-2">
                            <a href="{{ route('categorias-produtos.edit', $categoria) }}" class="text-brand-500 hover:underline">Editar</a>
                            <form action="{{ route('categorias-produtos.destroy', $categoria) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta categoria?')">
                                @csrf
                                @method('DELETE')
...
@if(function_exists('do_action'))
<?php do_action('entity.index.after_table', 'categoria_produto'); ?>
@endif
@endsection
