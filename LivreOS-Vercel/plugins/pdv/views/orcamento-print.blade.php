...
@php
    // Empresa e logotipo são do sistema principal — não dependem de cliente selecionado
    ...
    // Logo: mesma lógica do PDV api/imagem?path=... — na impressão usamos base64 inline (janela about:blank não envia cookies na requisição da img)
    ...
    if (!$empresaLogoBase64 && !$empresaLogoUrl && $empresa?->logo_path) {
        ...
        if ($logoPathRel !== '' && \Illuminate\Support\Facades\Storage::disk('public')->exists($logoPathRel)) {
            ...
            $empresaLogo = route('plugin.pdv.api.imagem', [], true) . '?path=' . rawurlencode($logoPathRel);
        }
    }
    ...
    $desconto = (float) ($desconto ?? 0);
    $observacao = $observacao ?? '';
    $dataEmissao = $dataEmissao ?? now()->format('d/m/Y H:i');
    $numeroOrcamento = $numeroOrcamento ?? null;
    if ($numeroOrcamento !== null && (string) $numeroOrcamento === '') $numeroOrcamento = null;
...
<head>
    <meta charset="utf-8">
    <title>Orçamento - {{ $clienteNome }}</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* Cabeçalho no estilo OS: sem bordas na tabela, logo alinhado ao centro vertical */
        ...
        .observacao { border: 1px solid #e5e7eb; padding: 10px; background: #f9fafb; white-space: pre-wrap; }
    </style>
</head>
<body>
    <div class="page">
        ...
        <div class="titulo">
            <table class="titulo-table"><tr>
                <td><h2>ORÇAMENTO @if($numeroOrcamento !== null && $numeroOrcamento !== '') nº {{ $numeroOrcamento }}@endif</h2></td>
                <td class="titulo-right"><span>Emissão: {{ $dataEmissao }}</span></td>
            </tr></table>
        </div>
            ...
            <table class="tabela-itens">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            {{ $item['descricao'] ?? '-' }}
                            @if(!empty($item['observacao'] ?? null))
                                <br><span style="font-size: 11px; color: #6b7280;">Obs: {{ $item['observacao'] }}</span>
                            @endif
                        </td>
