...
@section('content')
<div class="mb-6">
    <h1 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Editar Pagamento Recorrente</h1>
    <p class="text-gray-600 dark:text-gray-400">Altere os dados do recebimento recorrente</p>
</div>
...
<div class="rounded-lg border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-800">
    <form action="{{ route('financeiro.pagamentos-recorrentes.update', $recorrente) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
                @include('partials.cliente-autocomplete-field', [
                    ...
                    'clientes' => $clientes,
                    'selectedId' => old('cliente_id', $recorrente->cliente_id),
                    'idSuffix' => '_edit',
                    'label' => 'Cliente',
                ])
            </div>
            ...
            <div>
                ...
                <select name="tipo" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    ...
                    <option value="contrato" {{ old('tipo', $recorrente->tipo) === 'contrato' ? 'selected' : '' }}>Contrato</option>
                    <option value="assinatura" {{ old('tipo', $recorrente->tipo) === 'assinatura' ? 'selected' : '' }}>Assinatura</option>
                    <option value="manutencao" {{ old('tipo', $recorrente->tipo) === 'manutencao' ? 'selected' : '' }}>Manutenção</option>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Atualizado automaticamente após cada geração</p>
            </div>
            <div>
        ...
        <div class="mt-8 rounded-lg border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-800 dark:bg-blue-900/10">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="controle_origem" maxlength="60" value="{{ old('controle_origem', $recorrente->controle_origem) }}"
                ...
                <div>
                    ...
                    <input type="text" name="controle_entidade_tipo" maxlength="80" value="{{ old('controle_entidade_tipo', $recorrente->controle_entidade_tipo) }}"
                ...
                <div>
                    ...
                    <input type="text" name="controle_entidade_id" maxlength="120" value="{{ old('controle_entidade_id', $recorrente->controle_entidade_id) }}"
                ...
                <div>
                    ...
                    <input type="text" name="controle_referencia" maxlength="120" value="{{ old('controle_referencia', $recorrente->controle_referencia) }}"
        ...
        <div class="mt-8 rounded-lg border border-amber-200 bg-amber-50/50 p-4 dark:border-amber-800 dark:bg-amber-900/10">
            <div class="mb-3 flex items-center gap-2">
                <input type="checkbox" name="reajuste_habilitado" id="reajuste_habilitado" value="1" {{ old('reajuste_habilitado', $recorrente->reajuste_habilitado ?? false) ? 'checked' : '' }} class="rounded border-gray-300 text-brand-500 focus:ring-brand-500">
            ...
            <p class="mb-4 text-xs text-amber-800 dark:text-amber-200">Permite atualizar o valor após X períodos, por índice (IGPM, IPCA, etc.) ou percentual manual, com opção de desconto sobre o reajuste.</p>
            <div id="reajuste_campos" class="{{ old('reajuste_habilitado', $recorrente->reajuste_habilitado ?? false) ? '' : 'hidden' }} grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-xs font-medium text-gray-700 dark:text-gray-300">Reajustar a cada (meses)</label>
                    <input type="number" name="reajuste_periodo_meses" value="{{ old('reajuste_periodo_meses', $recorrente->reajuste_periodo_meses) }}" min="1" max="120" placeholder="Ex.: 12 (anual)" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
                <div>
                    ...
                    <select name="reajuste_tipo" id="reajuste_tipo" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                        @foreach(\App\Models\PagamentoRecorrente::reajusteTipos() as $v => $l)
                            <option value="{{ $v }}" {{ old('reajuste_tipo', $recorrente->reajuste_tipo ?? 'nenhum') === $v ? 'selected' : '' }}>{{ $l }}</option>
                        @endforeach
                    </select>
                </div>
                <div id="reajuste_indice_wrap" class="{{ old('reajuste_tipo', $recorrente->reajuste_tipo) === 'indice' ? '' : 'hidden' }}">
                    ...
                    <select name="reajuste_indice" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                        @foreach(\App\Models\PagamentoRecorrente::reajusteIndices() as $v => $l)
                            <option value="{{ $v }}" {{ old('reajuste_indice', $recorrente->reajuste_indice) === $v ? 'selected' : '' }}>{{ $l }}</option>
                        @endforeach
                    </select>
                </div>
                <div id="reajuste_manual_wrap" class="{{ old('reajuste_tipo', $recorrente->reajuste_tipo) === 'manual' ? '' : 'hidden' }}">
                    <label class="mb-1.5 block text-xs font-medium text-gray-700 dark:text-gray-300">Percentual manual (%)</label>
                    <input type="number" step="0.01" name="reajuste_manual_pct" value="{{ old('reajuste_manual_pct', $recorrente->reajuste_manual_pct) }}" min="0" max="999.99" placeholder="Ex.: 5,5" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
                <div>
                    <label class="mb-1.5 block text-xs font-medium text-gray-700 dark:text-gray-300">Desconto sobre o reajuste (%)</label>
                    <input type="number" step="0.01" name="reajuste_desconto_pct" value="{{ old('reajuste_desconto_pct', $recorrente->reajuste_desconto_pct) }}" min="0" max="100" placeholder="Ex.: 10 = aplicar 90% do índice" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                </div>
            </div>
        </div>
