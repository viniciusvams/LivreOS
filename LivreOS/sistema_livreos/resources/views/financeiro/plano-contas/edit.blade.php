...
@section('content')
<div class="mb-6">
    <h1 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Editar Conta</h1>
...
<div class="rounded-lg border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-800">
    <form action="{{ route('financeiro.plano-contas.update', $conta) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="codigo" value="{{ old('codigo', $conta->codigo) }}" placeholder="Ex: 1.1.1" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>
            ...
            <div>
                ...
                <select name="tipo" required class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    <option value="receita" {{ old('tipo', $conta->tipo) === 'receita' ? 'selected' : '' }}>Receita</option>
                    <option value="despesa" {{ old('tipo', $conta->tipo) === 'despesa' ? 'selected' : '' }}>Despesa</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="categoria" required class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    <option value="operacional" {{ old('categoria', $conta->categoria) === 'operacional' ? 'selected' : '' }}>Operacional</option>
                    <option value="nao_operacional" {{ old('categoria', $conta->categoria) === 'nao_operacional' ? 'selected' : '' }}>Não Operacional</option>
                    <option value="financeira" {{ old('categoria', $conta->categoria) === 'financeira' ? 'selected' : '' }}>Financeira</option>
                </select>
            </div>

            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Ordem</label>
                <input type="number" name="ordem" value="{{ old('ordem', $conta->ordem) }}" min="0" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
            </div>

            <div class="md:col-span-2">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Descrição</label>
                <textarea name="descricao" rows="3" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('descricao', $conta->descricao) }}</textarea>
            </div>
        ...
        <?php do_action('financeiro.plano-contas.form.extra', $conta); ?>
        @endif
