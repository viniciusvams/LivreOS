...
<body>
    <h1>{{ $empresaNome }}</h1>
    <p class="subtitulo">Cadastro de Serviços — Gerado em {{ now()->format('d/m/Y H:i') }}</p>
    ...
    @else
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $s->codigo_interno ?? '—' }}</td>
                    <td>{{ $s->nome }}</td>
                    <td>{{ $s->codigo_sku ?? '—' }}</td>
                    <td>{{ $s->unidade ?? '—' }}</td>
                    <td class="text-right">{{ $s->preco ? number_format($s->preco, 2, ',', '.') : '—' }}</td>
                    <td>{{ $s->categoria?->nome ?? '—' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        <p class="subtitulo" style="margin-top: 10px;">Total: {{ $servicos->count() }} serviços</p>
    @endif
